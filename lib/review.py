...
class Review:
    ...
    def __repr__(self):
        return f"<Review {self.id}: {self.year}, {self.summary}, employee_id={self.employee_id}>"
    ...
    def save(self):
        """Insert new row into DB or update if exists"""
        if self.id is None:
            sql = """
    ...
    def instance_from_db(cls, row):
        """Return cached instance if exists, else create"""
